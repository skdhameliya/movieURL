...
app.get('/all_products1/:start_n', async(req, res) => {
    try{
        const start_n = req.params.start_n
...
app.get('/all_products/:start_n', async(req, res) => {
    try{
        const start_n = req.params.start_n
...
app.get('/product1/:p_id', async(req, res) => {
    try{
...
app.get('/product/:p_id', async(req, res) => {
    try{
        const id1 = req.params.p_id
        console.log(id1);
        const singleProduct = await product.find({id : { $regex: '.*' + id1 + '.*', $options: 'i'}})   //db vala key name : const vala variable name
        res.status(200).json(singleProduct)
    }
    catch (error){
        res.status(400).json(error) 
